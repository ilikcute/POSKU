<template>
    <AuthenticatedLayout>
        <Head title="Edit Retur Pembelian" />
        ...
        <template #header>
            <div class="flex flex-col gap-2">
                ...
                <p class="text-xs text-[#555]">Sesuaikan item retur pembelian.</p>
            </div>
        </template>
            ...
            <div class="bg-[#f7f7f7] border border-[#9c9c9c] rounded shadow-sm p-6 space-y-6">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div class="lg:col-span-2">
                        ...
                        <div class="text-xs text-[#555] mt-4 mb-2">Supplier</div>
                        <div class="bg-white border border-[#9c9c9c] rounded px-3 py-2 text-xs">
                    ...
                    <div class="bg-white border border-[#9c9c9c] rounded p-3 text-xs">
                        ...
                        <div class="font-semibold">{{ formatDate(purchaseReturn.return_date) }}</div>
                        <div class="text-[#555] mt-3 mb-1">User</div>
                        <div class="font-semibold">{{ purchaseReturn.user?.name || "-" }}</div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap justify-end gap-3">
                    <Link :href="route('purchase-returns.show', purchaseReturn.id)"
...
const form = useForm({
    items: [],
    payment_method: props.purchaseReturn?.payment_method || props.purchaseReturn?.purchase?.payment_method || "cash",
...
const existingDetails = computed(() =>
    props.purchaseReturn?.purchase_return_details ||
    props.purchaseReturn?.purchaseReturnDetails ||
    []
);
...
const fetchReturnables = async () => {
    const purchaseId = props.purchaseReturn?.purchase?.id;
    ...
    try {
        ...
        existingDetails.value.forEach((detail) => {
            ...
            if (existing) {
                ...
                existing.price_at_purchase =
                    existing.price_at_purchase || detail.price_at_return || 0;
            } else {
                map.set(key, {
                    product_id: key,
                    ...
                    price_at_purchase: detail.price_at_return || detail.price_at_purchase || 0,
                    quantity: quantity,
                });
            }
        });
...
const submitReturn = () => {
    ...
    form.patch(route("purchase-returns.update", props.purchaseReturn.id));
};
