    ...
    <AuthenticatedLayout>
        ...
        <template #header>
            <div class="flex flex-col gap-2">
                ...
                <p class="text-xs text-[#555]">Informasi lengkap retur pembelian.</p>
            </div>
        </template>
            ...
            <div class="bg-[#f7f7f7] border border-[#9c9c9c] rounded shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-[#9c9c9c] flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <Link :href="route('purchase-returns.edit', purchaseReturn.id)"
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white border border-[#9c9c9c] rounded p-4">
                            <h3 class="text-sm font-semibold mb-3 text-[#1f1f1f]">Referensi Pembelian</h3>
                            <div class="space-y-2 text-xs text-[#1f1f1f]">
                                <p><strong>Invoice:</strong> {{ purchaseReturn.purchase?.invoice_number || "-" }}</p>
                                <p><strong>Supplier:</strong> {{ purchaseReturn.purchase?.supplier?.name || "-" }}</p>
                                <p><strong>Total Retur:</strong> {{ formatCurrency(purchaseReturn.final_amount) }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="purchaseReturn.notes" class="bg-white border border-[#9c9c9c] rounded p-4">
                        <h3 class="text-sm font-semibold mb-2 text-[#1f1f1f]">Catatan</h3>
                        <p class="text-xs text-[#1f1f1f]">{{ purchaseReturn.notes }}</p>
                    </div>
                </div>
            </div>
        </div>
