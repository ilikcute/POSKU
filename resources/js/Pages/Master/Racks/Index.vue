...
const deleteItem = (id) => {
    if (confirm("Apakah Anda yakin ingin menghapus item ini?")) {
        router.delete(route("master.racks.destroy", id));
    }
};
</script>

<template>
    <Head title="Master Rack" />
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wide" style="font-family: 'Poppins', sans-serif;">Nama Rak</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="rack in racks.data" :key="rack.id" class="hover:bg-gray-50">
        ...
        <Modal :show="isModalOpen" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900">
                    {{ isEditMode ? "Edit Rack" : "Tambah Rack" }}
                </h2>
                <form @submit.prevent="saveItem" class="mt-6 space-y-4">
                    <div>
                        <InputLabel for="name" value="Nama Rack" />
                        <TextInput
                            id="name"
                            v-model="form.name"
