<template>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm text-gray-200 divide-y divide-gray-600">
                            <thead class="bg-gray-800/70">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Kode Customer</th>
                            ...
                            <tbody class="divide-y divide-gray-700">
                                <tr v-for="customer in customers.data" :key="customer.id" class="hover:bg-gray-700/50 transition-colors duration-150">
                                    <td class="px-4 py-3 whitespace-nowrap font-mono text-sm">{{ customer.member_code }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap font-medium text-gray-200">{{ customer.name }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap text-gray-400">{{ customer.phone || '-' }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap truncate max-w-xs text-gray-400" :title="customer.address">{{ customer.address || '-' }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <button title="Cetak Kartu Customer" @click="printCard(customer)" class="inline-block bg-gradient-to-r from-blue-400 to-sky-400 text-white font-bold py-2 px-4 rounded-full text-xs shadow-lg hover:scale-105 hover:from-blue-500 hover:to-sky-500 transition-transform duration-200">
                                            ...
                                            <button @click="openModal(true, customer)" class="inline-block bg-gradient-to-r from-blue-400 to-blue-500 text-white font-bold py-2 px-4 rounded-full text-xs shadow-lg hover:scale-105 hover:from-blue-500 hover:to-blue-600 transition-transform duration-200">
                                                <svg class="w-3 h-3 mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <button type="button" @click="closeModal"
                            ...
                            <button type="submit" :disabled="form.processing"
                                class="inline-block bg-gradient-to-r from-green-400 to-emerald-400 text-white font-bold py-2 px-6 rounded-full text-sm shadow-lg hover:scale-105 hover:from-green-500 hover:to-emerald-500 transition-transform duration-200" :class="{ 'opacity-25': form.processing }">
                                <svg class="w-3 h-3 mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <CustomerCard :show="showCustomerCard" :customer="selectedCustomerForCard" :store="store" @close="closeCustomerCard" />
    </AuthenticatedLayout>
</template>
...
@media print {
    ...
    .customer-card-container, .customer-card-container * {
        visibility: visible !important;
    }
    #customer-card, #customer-card * {
        visibility: visible !important;
    }
    .fixed, .inset-0, .overflow-y-auto, .min-h-screen, .pt-4, .px-4, .pb-20, .text-center, .sm\:block, .sm\:p-0 {
