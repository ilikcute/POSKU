        ...
        <template #header>
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    ...
                    <p class="text-xs text-[#555]">Cocokkan stok sistem dengan perhitungan fisik.</p>
                </div>
                    ...
                    <tbody class="divide-y divide-[#d0d0d0]">
                        <tr v-for="stock in stocks.data" :key="stock.id" class="hover:bg-[#f7f7f7]">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium">{{ stock.product?.name || 'Unknown Product' }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="number" v-model="stock.physical_count" @input="calculateDifference(stock)"
                                    class="w-24 border border-[#9c9c9c] rounded px-2 py-1 text-sm bg-white text-[#1f1f1f]">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                {{ stock.difference || 0 }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    :class="stock.difference === 0 ? 'bg-[#e1f3e1] text-[#1f7a1f]' : 'bg-[#ffe1e1] text-[#8a1f1f]'"
